...
public class Potenciador3 extends Thread{
    Partida p;
    ...
    public Potenciador3(Partida p) {
        this.p = p;
        label=new JLabel();
        int posy=(int) (Math.random()*685+25);
        label.setBounds(1170,posy,40,40);
        ImageIcon imagen=new ImageIcon("src/imagenes/potenciador 3.png");
        ...
        try {
            while (true) {
                ...
                if(p.lnave.getBounds().intersects(label.getBounds())){
                    p.navepotenciada=true;
                    p.actualizar();
                    p.jPanel2.remove(label);
                    sleep(10000);
                    p.navepotenciada=false;
                    p.velpotenciada=1;
                    this.stop();
                }
